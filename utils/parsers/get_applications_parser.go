...
func ParseJxGetApplications(s string) (map[string]Application, error) {
	...
	for _, line := range lines {
		...
		if len(fields) == 4 {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "cannot convert %s to integer, entire output was %s", desiredPods, s)
			}
			runningPods, err = strconv.Atoi(pods[0])
			if err != nil {
				return nil, errors.Wrapf(err, "cannot convert %s to integer, entire output was %s", runningPods, s)
			}
		}
