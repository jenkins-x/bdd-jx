...
var _ = Describe("Import Application", func() {
	...
	Describe("Importing an application", func() {
		Context("by running jx import", func() {
			...
			It("creates a Go application from the specified folder and promotes it to staging", func() {
				destDir := T.WorkDir + "/" + T.ApplicationName
				url := "https://github.com/jenkins-x-quickstarts/golang-http.git"
				...
				By("updating the pom.xml to have the correct application name", func() {
					err := utils.ReplaceElement(filepath.Join(destDir, "Makefile"), "REPLACE_ME_APP_NAME", T.ApplicationName, 1)
					Expect(err).NotTo(HaveOccurred())
				})
