	...
	Describe("Given valid parameters", func() {
		Context("when running import", func() {
			It("creates an app from the specified folder and promotes it to staging\n", func() {
				...
				c := "jx"
				args := []string{"import", dest_dir, "-b", "--org", T.GetGitOrganisation(), "--git-provider-url", T.GetGitProviderURL()}
				...
				if T.DeleteRepos() {
					By("deletes the repo")
					args = []string{"delete", "repo", "-b", "-g", T.GetGitProviderURL(), "-o", T.GetGitOrganisation(), "-n", T.AppName}
					command = exec.Command(c, args...)
					command.Dir = dest_dir
					session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
					Ω(err).ShouldNot(HaveOccurred())
